...
class RTAI_WC_Composer {
    ...
    public function generate_content($post_id, $artifacts, $context_overrides = array()) {
        ...
        if (!$product) {
            throw new Exception(__('Product not found.', 'rapidtextai-woocommerce'));
        }
        
        // Validate artifacts
        $artifacts = array_intersect($artifacts, self::SUPPORTED_ARTIFACTS);
        if (empty($artifacts)) {
            throw new Exception(__('No valid artifacts specified.', 'rapidtextai-woocommerce'));
        }
    ...
    public function apply_content($post_id, $artifact, $content) {
        ...
        if (!empty($flags['locked'][$artifact])) {
            throw new Exception(__('This field is locked from AI updates.', 'rapidtextai-woocommerce'));
        }
    ...
    public function rollback_content($post_id, $history_id) {
        ...
        if (!$entry) {
            throw new Exception(__('History entry not found.', 'rapidtextai-woocommerce'));
        }
    ...
    public function extract_attributes($post_id, $description) {
        ...
        try {
            ...
            throw new Exception(__('Failed to extract attributes: ', 'rapidtextai-woocommerce') . $e->getMessage());
        }
    }
}